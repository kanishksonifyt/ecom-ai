...
export const getReviewByIdStep = createStep(
    ...
    async (input: GetReviewByIdStepInput, { container }) => {
        ...
        const review = await reviewService.listReviews(input.id);

        return new StepResponse(review, review[0]?.id);
    }
);
...
export const getAllReviewsStep = createStep(
    ...
    async (_, { container }) => {
        ...
        return new StepResponse(reviews, reviews.map(review => review.id));
    }
);
