import { deleteHomeWorkflow  ,editHomeWorkflow , reorderHomesWorkflow , getHomeByIdWorkflow } from "../../../../workflows/create-home";
...
export const PUT = async (
    ...
    const updatedHero = await editHomeWorkflow(req.scope).run({ input: { id, ...parsedBody } });
...
export const PATCH = async (req: MedusaRequest<PatchAdminReorderHomeType>, res: MedusaResponse) => {
    const { id } = req.params;
    const { body } = req;
    ...
    if (typeof body === 'object' && body !== null) {
        reorderedHeroes = await reorderHomesWorkflow(req.scope).run({ input: { id, newIndex: parsedBody.newIndex } });
    }
