import { deleteHeroWorkflow  ,editHeroWorkflow , reorderHeroesWorkflow } from "../../../../workflows/create-hero";
...
export const PUT = async (
    ...
    const updatedHero = await editHeroWorkflow(req.scope).run({ input: { id, ...parsedBody } });
...
export const PATCH = async (req: MedusaRequest<PatchAdminReorderHeroType>, res: MedusaResponse) => {
    const { id } = req.params;
    const { body } = req;
    ...
    if (typeof body === 'object' && body !== null) {
        reorderedHeroes = await reorderHeroesWorkflow(req.scope).run({ input: { id, newIndex: parsedBody.newIndex } });
    }
